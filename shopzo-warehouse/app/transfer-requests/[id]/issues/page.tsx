...
const TransferIssuesPage = () => {
    ...
    const fetchIssues = async () => {
        if (!transferId || !warehouse?._id) return;
        setLoading(true);
        setError(null);
        try {
        ...
        try {
            await axios.patch(
                API_ENDPOINTS.RESOLVE_TRANSFER_ISSUE,
                {
                    ...
                    resolvedByType: "warehouse",
                    resolvedById: warehouse._id,
                },
                { withCredentials: true }
            );
            await fetchIssues();
        ...
        <div className="min-h-screen bg-gray-50 px-4 py-8 dark:bg-slate-900">
            <div className="mx-auto max-w-5xl space-y-4">
                <div className="flex items-center justify-between">
                    <div>
                        <Link href={`/transfer-requests/${transferId}`} className="text-sm text-gray-700 underline dark:text-slate-200">
